...
function OS:new(o, cid, event_handlers, websocket_url, turtle)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    ...
    self.appdata = {
        websocket_url = websocket_url or nil,
        retry_timer_id = nil
    }
    self.api = {
    ...
    self:addHandlers(event_handlers or {})
...
function OS:addHandler(event, handler)
    ...
    local eventHandlers = self.event_handlers[event] or {}
...
function OS:loadPrograms()
    if fs.isDir("programs") then
        ...
        for _, programName in ipairs(programFiles) do
...
function OS:run(shell)
    ...
    http.websocketAsync(self.appdata.websocket_url)
    ...
    local function systemLoop()
        while true do
            ...
            for _, eventHandler in pairs(self.event_handlers[eventName] or {}) do
...
function BuildWebsocketHandlers(system)
